...
using Progetto_GestioneEventi.DAL;
...
namespace Progetto_GestioneEventi.Classes
{
    internal class Sottomenu
    {
        ...
        {
            string stringa = "\n";
            ICollection<Evento> eventi = EventoDAL.getInstance().findAllgetPartecipanti();

            foreach(Evento evento in eventi)
            {
                stringa += $"{evento.Nome}\n";
                foreach(Partecipante partecipante in evento.PartecipanteRifs)
                {
        ...
        {
            string stringa = "\n";
            ICollection<Evento> eventi = EventoDAL.getInstance().findAllgetRisorse();

            foreach (Evento evento in eventi)
            {
                stringa += $"{evento.Nome}\n";
                foreach (Risorsa risorsa in evento.RisorsaRifs)
                {
                ...
                if (risposta == 1)
                {
                    ...
                    if (!partecipanti.Equals("Nessun partecipante trovato"))
                    {
                        ...
                        if (!eventi.Equals("Nessun evento trovato"))
                        {
                            ...
                            bool controllo = PartecipanteDAL.getInstance().assignPartecipante(idPartecipante, evento);

                            if (controllo)
                                Console.WriteLine("Assegnamento riuscito");
                        }
                    }
                }
            }
            ...
            try
            {
                ...
                if (risposta == 1)
                {
                    ...
                    if (!partecipanti.Equals("Nessun partecipante trovato"))
                    {
                        ...
                        Partecipante partecipante = PartecipanteDAL.getInstance().findByIDgetEventi(idPartecipante);

                        if (partecipante.EventoRifs != new List<Evento>())
                        {
                            Console.WriteLine();
                            foreach (Evento evento in partecipante.EventoRifs)
                            {
                        ...
                        bool controllo = PartecipanteDAL.getInstance().RemovePartecipante(e, partecipante);
        ...
        {
            string stringa = "\n";
            ICollection<Partecipante> partecipanti = PartecipanteDAL.getInstance().findAllgetEventi();
                ...
                if (risposta == 1)
                {
                    ...
                    if (!risorse.Equals("Nessuna risorsa trovata"))
                    {
                        ...
                        if (!eventi.Equals("Nessun evento trovato"))
                        {
                            ...
                            bool controllo = RisorsaDAL.getInstance().assignRisorsa(idRisorsa, evento);
        ...
        {
            string stringa = "\n";
            ICollection<Risorsa> risorse = RisorsaDAL.getInstance().findAllgetEventi();

            foreach (Risorsa risorsa in risorse)
            {
                stringa += $"{risorsa.Tipo} {risorsa.Nome}\n";
                foreach (Evento evento in risorsa.EventoRifs)
                {
                    stringa += $"Id:{evento.EventoId} | Nome:{evento.Nome} | Data:{evento.DataEvento} | Luogo:{evento.Luogo} | Capacità:{evento.Capacita}\n";
                }
            }
            ...
            try
            {
                ...
                if (risposta == 1)
                {
                    ...
                    if (!risorse.Equals("Nessuna risorsa trovata"))
                    {
                        ...
                        Risorsa risorsa = RisorsaDAL.getInstance().findByIDgetEventi(idRisorsa);

                        if (risorsa.EventoRifs != new List<Evento>())
                        {
                            Console.WriteLine();
                            foreach (Evento evento in risorsa.EventoRifs)
                            {
                        ...
                        bool controllo = RisorsaDAL.getInstance().RemoveRisorsa(e, risorsa);

                        if (controllo)
                            Console.WriteLine("Rimossa con successo");
                    }
                }
            }
            catch { Console.WriteLine("Qualcosa è andato storto."); }
        ...
        public void saveCSV()
        {
            try
            {
                using (StreamWriter sw = new StreamWriter("C:\\Users\\Utente\\Desktop\\Eventi.txt"))
                {
                    ...
                    foreach (Evento evento in eventi)
                    {
                        sw.WriteLine(evento.ToCSV());
                    }
                }
            }
            catch { Console.WriteLine("Eventi.CSV non salvato"); }

            try
            {
                using (StreamWriter sw = new StreamWriter("C:\\Users\\Utente\\Desktop\\Partecipanti.txt"))
                {
                    ICollection<Partecipante> partecipanti = PartecipanteDAL.getInstance().findAll();

                    foreach (Partecipante partecipante in partecipanti)
                    {
                        sw.WriteLine(partecipante.ToCSV());
                    }
                }
            }
            catch { Console.WriteLine("Partecipanti.CSV non salvato"); }

            try
            {
                using (StreamWriter sw = new StreamWriter("C:\\Users\\Utente\\Desktop\\Risorse.txt"))
                {
                    ICollection<Risorsa> risorse = RisorsaDAL.getInstance().findAll();

                    foreach (Risorsa risorsa in risorse)
                    {
                        sw.WriteLine(risorsa.ToCSV());
                    }
                }
            }
            catch { Console.WriteLine("Risorse.CSV non salvato"); }
        }
        #endregion
    }
}
